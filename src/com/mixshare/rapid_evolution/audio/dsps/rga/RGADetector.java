...
public class RGADetector {
    ...
    private static Logger log = Logger.getLogger(RGADetector.class);
    
    private static int blockSize = 50; // in milliseconds
    ...
    public RGAData detectRGA(String filename) {
        RGAData result = null;
        AudioDecoder decoder = null;
        try {
            if (log.isDebugEnabled())
                log.debug("detectRGA(): filename=" + filename);
            ...
            if (decoder != null) {
                if (InitGainAnalysis((int)decoder.getSampleRate()) == INIT_GAIN_ANALYSIS_OK) {
                    ...
                    int numChannels = decoder.getAudioFormat().getChannels();
                    AudioBuffer buffer = new AudioBuffer(chunkSize, numChannels);
                    int bufferIndex = 0;
                    boolean done = false;
                        ...
                        if (frames_read == 0) {
                            ...
                            for (int i = 0; i < frames_read; ++i) {
                                for (int c = 0; c < numChannels; ++c) {
                                    buffer.setSampleValue(bufferIndex, c, readBuffer.getSampleData(c)[i]);   
                                }                                         
                                ++bufferIndex;
                                if (bufferIndex >= buffer.getSampleData(0).length) {
    ...
    private static double GAIN_NOT_ENOUGH_SAMPLES = -24601.0;
    private static int GAIN_ANALYSIS_ERROR = 0;
    private static int GAIN_ANALYSIS_OK = 1;

    private static int INIT_GAIN_ANALYSIS_ERROR = 0;
    private static int INIT_GAIN_ANALYSIS_OK = 1;   
    
    private static int YULE_ORDER = 10;    
    private static int BUTTER_ORDER = 2;
    private static double RMS_PERCENTILE = 0.95;           // percentile which is louder than the proposed level
    private static double MAX_SAMP_FREQ = 48000.0;          // maximum allowed sample frequency [Hz]
    private static double RMS_WINDOW_TIME = 0.050;          // Time slice size [s]
    private static int STEPS_per_dB = 100;                  // Table entries per dB
    private static int MAX_dB = 120;                        // Table entries for 0...MAX_dB (normal max. values are 70...80 dB)

    private static int MAX_ORDER = (BUTTER_ORDER > YULE_ORDER ? BUTTER_ORDER : YULE_ORDER);
    private static int MAX_SAMPLES_PER_WINDOW = (int)(MAX_SAMP_FREQ * RMS_WINDOW_TIME);      // max. Samples per Time slice // (size_t)
    private static double PINK_REF = 64.82;                 //298640883795                              // calibration value
...
//     either ignore the warnings or uncomment the three "y" lines (and comment out the indicated line)
    ...
    static void filterYule (double[] input, int inputIndex, double[] output, int outputIndex, int nSamples, double[] kernel) {
       // if (log.isTraceEnabled())
            //log.trace("filterYule(): input length=" + input.length + ", inputIndex=" + inputIndex + ", output length=" + output.length + ", outputIndex=" + outputIndex + ", nSamples=" + nSamples);
        try {
            while (nSamples > 0) {
                output[outputIndex] =  
            ...
            log.error("filterYule(): error", aob);
            log.error("filterYule(): inputIndex=" + inputIndex + ", outputIndex=" + outputIndex);
            throw aob;
        }
    }
...
//        if (log.isTraceEnabled())
//            log.trace("AnalyzeSamples(): num_samples=" + num_samples);
