...
public class ExcelAdapter implements ActionListener {
    ...
    private static Logger log = Logger.getLogger(ExcelAdapter.class);
...
//                JComponent.WHEN_FOCUSED);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().compareTo("Paste") == 0) {
            log.debug("actionPerformed(): trying to paste");
            int startRow = (jTable1.getSelectedRows())[0];
            int startCol = (jTable1.getSelectedColumns())[0];
            try {
                String trstring = (String) (system.getContents(this)
                        .getTransferData(DataFlavor.stringFlavor));
                log.debug("actionPerformed(): string is=" + trstring);
                ...
                for (int i = 0; st1.hasMoreTokens(); i++) {
                    ...
                    for (int j = 0; st2.hasMoreTokens(); j++) {
                        ...
                        log.debug("actionPerformed(): putting " + value
                                + ", at row=" + startRow + i + ", column="
                                + startCol + j);
                    }
                }
            } catch (Exception ex) {
                log.error("actionPerformed(): error Exception", ex);
            }
        }
    }
}